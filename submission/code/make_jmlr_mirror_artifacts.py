...
def figure_convergence():
    """STK Fig. 2 analogue: convergence of the truncated operator via d_eff(n)."""
...
def figure_robustness():
    """STK Fig. 4 analogue: robustness across seeds (a) ratio, (b) Q_0.98, (c) per-family."""
...
def figure_per_instance():
    """STK Fig. 5 analogue: per-instance output grid (ratio per method x graph)."""
...
def figure_query_gap():
    """STK Fig. 7 analogue: pointwise optimality gap 1 - best-so-far ratio vs query index."""
    t = pd.read_csv(os.path.join(TAB, "trace_all_evaluations.csv"))
    # Map only the policies that correspond unambiguously to the manuscript baselines.
...
def table_prior_summary():
    """STK Table 1 analogue: qualitative summary of existing and proposed priors."""
    rows = [
    ...
    lines = [
        ...
        r"\toprule",
        r" & Graph operator & Noncommuting & Off-diagonal & Query- \\",
        r"Prior / policy & used & generators & search & ranked \\",
        r"\midrule",
    ]
